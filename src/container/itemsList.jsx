...
function ItemsList ({ items, setItems }) {
  ...
  function markAsBought (id) {
    // Note: If you are in the "Pending" tab and mark an item as bought,
    // it will naturally move to the "Bought" tab.
    setItems(prevItems =>
      prevItems.map(item =>
        item.id === id
          ? {
  ...
  // We use "items" directly here because the parent component (Content.js)
  // already passes the "filteredItems" as the "items" prop.
  return (
    <div>
      {items.map(item => (
        <div
        ...
        >
          <div className='col-span-4 text-[1.1rem] uppercase text-[#333446]'>
            {item.name}
          </div>
          ...
          <div className='col-span-2 text-[#333446] font-semibold'>
            ₱{item.cost}
          </div>
          <div className='flex col-span-1 justify-end'>
            <div
              onClick={() => markAsBought(item.id)}
              className={`items-list-mark-button cursor-pointer
                ${
                  item.status === 'MARK'
            ...
            <div
              ...
              className='items-list-clear cursor-pointer'
            >
              ☓
            </div>
          </div>
          ...
          <div className='flex items-start gap-3'>
            <div
              className={`w-2 h-full min-h-[60px] rounded-full ${
                item.status === 'BOUGHT' ? 'bg-green-400' : 'bg-blue-600'
              }`}
            />
            ...
            <div className='flex-1 min-w-0'>
              <div className='gap-2 flex justify-between items-start mb-2'>
                <div className='w-4/7'>
                  ...
                  <span className='text-xs text-[#273F4F]/70 mt-0.5 block'>
                    {item.category}
                  </span>
                </div>
              ...
              <div className='flex items-center justify-between text-sm'>
                <div className='flex items-center gap-4'>
                  <div className='text-[#333446]'>
                    <span className='block text-xs text-[#273F4F]/70'>
                      Quantity
                    </span>
